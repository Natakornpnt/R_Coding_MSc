...
#			It is value must be specified as an integer >= 3
#
###RESULTS
#
##STEPWISE TABLE
...
plot_step_RSS<-function(x){
	...
	plot(0,0,xlim=c(0,nrow(x$RSSout)-1),ylim=c(0,1),xlab='step',ylab='%var',col=0, main='Partition of variance at each step of MLMM')
...
#	plot_col<-rep(c('gray10','gray60'),ceiling(max(unique(output_ok$Chr))/2))
	plot_col<-rainbow(12, start=.7, end=.1)
	...
	plot(output_ok$xpos,-log10(output_ok$pval),col=output_ok$col,pch=20,ylab='-log10(pval)',xaxt='n',xlab='Chromosome', main="Manhattan plot", ylim=c(0,14))
...
plot_region<-function(x,chrom,pos1,pos2){
	...
	region$col<- if (chrom %% 2 == 0) {'grey60'} else {'gray10'}
	region$col[which(region$SNP %in% x$cof)]<-'red'
	plot(region$Pos,-log10(region$pval),type='p',pch=20,main=paste('Manhattan plot for chromosome',chrom,sep=' '),xlab='position (bp)',ylab='-log10(pval)',col=region$col,xlim=c(pos1,pos2), ylim=c(0,14))
	abline(h=x$bonf_thresh,lty=3,col='black')}
...
qqplot_opt_GWAS<-function(x,opt){
	if (opt=='extBIC') {
		...
		maxp<-ceiling(max(o))
		plot(e,o,type='b',pch=20,cex=0.8,col="dodgerblue4",xlab=expression(Expected~~-log[10](italic(p))), ylab=expression(Observed~~-log[10](italic(p))),xlim=c(0,max(e)+1),ylim=c(0,maxp),main=paste('QQplot for optimal model according to extBIC'))
		abline(0,1,col="black")}
	else if (opt=='mbonf') {
		e<--log10(ppoints(nrow(x$opt_mbonf$out)))
		o<--log10(sort(x$opt_mbonf$out$pval))
		maxp<-ceiling(max(o))
		plot(e,o,type='b',pch=20,cex=0.8,col="dodgerblue4",xlab=expression(Expected~~-log[10](italic(p))), ylab=expression(Observed~~-log[10](italic(p))),xlim=c(0,max(e)+1),ylim=c(0,maxp),main=paste('QQplot for Optimal model according to mbonf'))
